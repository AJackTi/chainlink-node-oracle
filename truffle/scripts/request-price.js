...
module.exports = async (callback) => {
  ...
  if (jobs.length !== 0) {
    ...
    try {
      let receipt = await client.requestEthereumPrice(oracle.address, jobID);
      console.log("TX receipt: ", receipt);
      console.log(
        `... waiting 10 seconds to allow chainlink off chain API call to return data`
      );
      await sleep(10000);
      ...
      console.log(
        "Failed to send Eth price request. Make sure client contract balance has enough LINK \nError:",
        e.name
      );
    }
  }
